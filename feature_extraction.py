...
# 3. Keypoints ve Descriptors  çıkarımı yap
keypoints_1, descriptors_1 = sift.detectAndCompute(img1, None)
keypoints_2, descriptors_2 = sift.detectAndCompute(img2, None)

# 4. knnMatch ile en yakın 2 noktayı bul ve Lowe's Ratio Test uygula
bf = cv2.BFMatcher()
matches = bf.knnMatch(descriptors_1, descriptors_2, k=2)

good_matches = []
for m, n in matches:
    # Eğer birinci eşleşme, ikinci eşleşmeden %75 daha yakınsa bunu "güvenilir" kabul et
...
# Sadece RANSAC'tan geçen (mask değeri 1 olan) noktaları yeşil renkle çiz
...
# 5. Sadece güvenilir (good) eşleşmeleri görselleştir
...
# Sadece RANSAC maskesinden geçen temiz noktaları al
# DOĞRU (int32 yerine float32)
...
# 2. Essential Matrix (Temel Matris) Hesaplama
E, mask_E = cv2.findEssentialMat(pts1_clean, pts2_clean, K, method=cv2.RANSAC, prob=0.999, threshold=1.0)

# 3. Kamera Pozisyonunu Çıkarma (R: Rotasyon Matrisi, t: Çeviri Vektörü)
_, R, t, mask_pose = cv2.recoverPose(E, pts1_clean, pts2_clean, K)

#4. Projeksiyon Matrislerini Oluşturma
Proj1 = np.hstack((np.eye(3), np.zeros((3, 1)))) # İlk kamera merkezde (0,0,0)
Proj2 = np.hstack((R, t)) # İkinci kamera R ve t kadar hareket etmiş
...
# Homojen koordinatları standart 3D (X, Y, Z) koordinatlarına çevir
points_3d = points_4d_hom[:3, :] / points_4d_hom[3, :]
points_3d = points_3d.T

# 6. 3D Nokta Bulutunu Görselleştir (Matplotlib 3D)
fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(111, projection='3d')

# Noktaları çiz (mavi renkli)
...
# Ekseni eşit oranlı yap (görsel bozulmayı engellemek için)
...
# 8. Derinlik Haritası (Disparity Map) Hesaplama - StereoSGBM Algoritması
window_size = 5
min_disp = 0
